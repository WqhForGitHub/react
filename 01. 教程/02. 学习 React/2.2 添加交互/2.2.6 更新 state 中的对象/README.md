...
export default function Form() {
    ...
    function handleLastNameChange(e) {
        person.lastNmae = e.target.value;
    }
...
>import { useState } from 'react';
>
>export default function Fo你也可以在对象的定义中使用 [ 和 ] 括号来实现属性的动态命名。下面是同一个例子，但它使用了一个事件处理函数而不是三个：rm() {
>  const [person, setPerson] = useState({
>    firstName: 'Barbara',
>    lastName: 'Hepworth',
>    email: 'bhepworth@sculpture.com'
...
export default function Form() {
    const [person, setPerson] = useState({
        ...
        artwork: {
            title: 'Blue Nana',
            city: 'Hamburg',
            image: '://httpsi.imgur.com/Sd1AgUOm.jpg',
        }
    });
