...
1. 创建一个新的 JS 文件来存在该组件。
2. 导出该文件中的函数组件（可以使用默认导出或具名导出）
3. 在需要使用该组件的文件中导入（可以根据相应的导出方式使用 [默认导入](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/import#importing_defaults) 或 [具名导入](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/import#import_a_single_export_from_a_module)）。
...
* 使用默认导出的方式，将 Gallery 组件导出

2. App.js

* 使用默认导入的方式，从 Gallery.js 中导入 Gallery 组件。
* 使用默认导出的方式，将根组件 App 导出。
...
* Gallery.js：
  * 使用具名导出的方式，将 Profile 组件导出，并取名为Profile。
  * 使用默认导出的方式，将 Gallery 组件导出。
* App.js：
  * 使用具名导入的方式，从 Gallery.js 中导入 Profile 组件，并取名为 Profile。
  * 使用默认导入的方式，从 Gallery.js 中导入 Gallery 组件。
  * 使用默认导出的方式，将根组件 App 导出。
